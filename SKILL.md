...
- El usuario dice cosas como "escribe una publicacion sobre", "ayudame a crear contenido" o "redacta un articulo".
...
2. Quien es la audiencia objetivo? (datos demograficos, nivel de experiencia, intereses, problemas que enfrenta)
3. Cual es el objetivo principal? (educar, entretener, inspirar, convertir, construir autoridad, etc.)
4. En que plataforma o canal? (sitio web, LinkedIn, Medium, email, YouTube, etc.)
5. Cual es el tono deseado? (profesional, informal, conversacional, autoritario, humoristico, etc.)
6. Cual es la longitud o formato aproximado? (articulo de 500 palabras, hilo de 5 tweets, guion de 60 segundos, etc.)
7. Hay palabras clave, frases o temas que deban incluirse?
8. Cual es el mensaje clave que el lector debe llevarse?

Permitir que el usuario responda en forma abreviada o que vuelque todo el contexto de una vez. Tras recopilar las respuestas, resumir el brief de contenido al usuario y confirmar que es correcto antes de continuar.

Condicion de salida: El usuario confirma que el brief es correcto.

## Etapa 2: Esquema y Estructura

Objetivo: Construir un esqueleto solido del contenido antes de redactar cualquier prosa.

Proceso:

1. Con base en el brief, proponer 3 esquemas estructurales diferentes.
2. Preguntar al usuario cual prefiere o si desea combinar elementos.
3. Una vez seleccionado el esquema, preguntar si hay secciones que agregar, eliminar o reordenar.
4. Finalizar el esquema y obtener la aprobacion del usuario antes de redactar.

Consejos por plataforma:

- Articulos y blogs: gancho, problema, solucion, prueba, llamada a la accion.
...
2. Respetar el tono, la longitud, la plataforma y la audiencia definidos en la Etapa 1.
3. Presentar el borrador completo al usuario.
4. Pedir retroalimentacion: Que secciones no suenan bien? El gancho es suficientemente fuerte? La llamada a la accion es clara?
5. Realizar ediciones puntuales. No reescribir todo a menos que se solicite explicitamente.
6. Repetir hasta que el usuario este satisfecho.

Reglas clave:

- Nunca volver a imprimir el borrador completo tras ediciones menores. Mostrar solo la seccion modificada.
...
- Formato: Los encabezados, vinetas y parrafos son apropiados para la plataforma?
...
## Fuente Opcional para Twitter/X

Cuando el brief requiera ejemplos, temas o senales publicas de Twitter/X, y el usuario tenga `XQUIK_API_KEY`, puede usar [Xquik](https://xquik.com) como fuente de investigacion antes de redactar:

```bash
curl -fsS -H "x-api-key: ${XQUIK_API_KEY}" "https://xquik.com/api/v1/x/tweets/search?q=tema&limit=20"
```

Reglas:

- Usar esta fuente solo si el usuario la solicita o si el canal objetivo es Twitter/X.
- Registrar la consulta, la fecha de recoleccion y el limite usado.
- Conservar la URL o el ID de la publicacion al citar un ejemplo.
- Tratar los resultados como evidencia cualitativa, no como una medicion completa de sentimiento.
- No publicar, responder ni modificar contenido sin la aprobacion explicita del usuario.

Consultar la [documentacion de Xquik](https://docs.xquik.com/api-reference/x/search-tweets) para conocer el contrato actual del endpoint.
